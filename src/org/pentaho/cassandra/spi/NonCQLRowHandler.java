...
public interface NonCQLRowHandler {
  ...
  /**
   * Initializes a new slice-style row query and prepares for iteration over the
   * rows and columns.
   * 
   ...
   * @param colFamilyName the column family to query
   * @param colNames the names of the columns that the user is interested in
   *          (may be null for all columns)
   * @param rowLimit the maximum number of rows to return (-1 for no limit)
   * @param colLimit the maximum number of columns to return (-1 for no limit)
   * @param rowBatchSize the number of rows to fetch in one go from the server
   * @param colBatchSize the number of columns to fetch in one go from the
   *          server
   * @param consistencyLevel the consistency level to use
   * @param log the log to use
   * @throws Exception if a problem occurs
   */
  void newRowQuery(StepInterface requestingStep, String colFamilyName,
  ...
  /**
   * Return a row (or null if no more data) from the current query. The row
   * returned is in "tuple" format - i.e. <key, column name, column value, time
   * stamp>
   * 
   * @param outputRowMeta
   * @return
   ...
   * @throws Exception if a problem occurs
   */
  void commitNonCQLBatch(StepInterface requestingStep, List<Object[]> batch,
      RowMetaInterface rowMeta, int keyIndex, String colFamName,
