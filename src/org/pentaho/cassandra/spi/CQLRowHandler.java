...
public interface CQLRowHandler {
  ...
  /**
   * Returns true if the underlying driver supports the provided CQL major
   * version number (e.g. 2 or 3).
   * 
   * @param cqMajorlVersion the major version number of the CQL version to check
   ...
   * 
   * @param requestingStep the step that is requesting the commit - clients can
   ...
   * @param compress name of the compression to use (may be null for no
   *          compression)
   * @param consistencyLevel the consistency level to use
   * @param log the log to use
   * @throws Exception if a problem occurs
   */
  void commitCQLBatch(StepInterface requestingStep, StringBuilder batch,
   ...
   *          applies to
   * @param inputMeta the structure of the incoming Kettle rows inserting
   ...
   * @throws Exception if a problem occurs
   */
  boolean addRowToCQLBatch(StringBuilder batch, String colFamilyName,
   ...
   * 
   * @param requestingStep the step that is requesting rows - clients can use
   *          this primarily to check whether the running transformation has
   *          been paused or stopped (via isPaused() and isStopped())
   ...
   * @throws Exception if a problem occurs
   */
  void newRowQuery(StepInterface requestingStep, String colFamName,
   ...
   *           occurs.
   */
  Object[] getNextOutputRow(RowMetaInterface outputRowMeta,
